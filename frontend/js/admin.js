(function(){
  ...
  function renderDetails(data){
    ...
    // Create editable final score field
    const finalScoreField = document.createElement("div");
    finalScoreField.className = "detail-grid";
    
    const finalScoreLabel = document.createElement("div");
    finalScoreLabel.className = "detail-label";
    finalScoreLabel.textContent = "Final Score:";
    
    const finalScoreValue = document.createElement("div");
    finalScoreValue.className = "detail-value";
    
    const finalScoreInput = document.createElement("input");
    finalScoreInput.type = "number";
    finalScoreInput.min = "1";
    finalScoreInput.max = "5";
    finalScoreInput.step = "0.1";
    finalScoreInput.value = data.final_score || "";
    finalScoreInput.placeholder = "Final Score (1-5)";
    finalScoreInput.style.width = "80px";
    finalScoreInput.style.padding = "2px 4px";
    finalScoreInput.style.marginRight = "8px";
    
    const saveFinalScoreBtn = document.createElement("button");
    saveFinalScoreBtn.textContent = "Save Final Score";
    saveFinalScoreBtn.style.fontSize = "11px";
    saveFinalScoreBtn.style.padding = "2px 6px";
    saveFinalScoreBtn.onclick = () => saveFinalScore(data.session_id, finalScoreInput.value);
    
    finalScoreValue.appendChild(finalScoreInput);
    finalScoreValue.appendChild(saveFinalScoreBtn);
    
    finalScoreField.appendChild(finalScoreLabel);
    finalScoreField.appendChild(finalScoreValue);
    
    infoGrid.appendChild(finalScoreField);
    ...
    for(let i=0;i<pairs.length;i++){
      ...
      scoreInput.placeholder = "Score (1-5)";
      scoreInput.style.width = "80px";
      scoreInput.style.padding = "4px";
      scoreInput.style.marginRight = "8px";
      
      const saveScoreBtn = document.createElement("button");
      saveScoreBtn.textContent = "Save";
      saveScoreBtn.style.fontSize = "12px";
      saveScoreBtn.style.padding = "4px 8px";
      saveScoreBtn.onclick = () => saveScore(data.session_id, i, scoreInput.value, feedbackInput.value);
      
      sd.appendChild(document.createTextNode("Score: "));
      sd.appendChild(scoreInput);
      sd.appendChild(saveScoreBtn);
      ...
      feedbackInput.style.height = "60px";
      feedbackInput.style.padding = "4px";
      feedbackInput.style.marginTop = "4px";
      ...
      saveFeedbackBtn.style.fontSize = "12px";
      saveFeedbackBtn.style.padding = "4px 8px";
      saveFeedbackBtn.style.marginTop = "4px";
      saveFeedbackBtn.onclick = () => saveScore(data.session_id, i, scoreInput.value, feedbackInput.value);
      
      fd.appendChild(document.createTextNode("Feedback: "));
      fd.appendChild(feedbackInput);
      fd.appendChild(saveFeedbackBtn);
...
See console for details.`);
  ...
  async function saveScore(sessionId, answerIndex, score, feedback) {
    if (!sessionId || answerIndex === undefined) {
      alert("Invalid session or answer index");
      return;
    }
    ...
    try {
      const res = await fetch("/admin/update-score", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        // Reload the details to show updated scores
  ...
  async function saveFinalScore(sessionId, finalScore) {
    if (!sessionId) {
    ...
    if (finalScore && (finalScore < 1 || finalScore > 5)) {
      alert("Final score must be between 1 and 5");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        alert("Final score updated successfully!");
        // Reload the sessions list to show updated scores
        load();
        // Reload the details to show updated scores
        loadDetails(sessionId);
      } else {
        alert("Error updating final score: " + data.error);
      }
    } catch (e) {
      alert("Error updating final score: " + e.message);
      console.error("Final score update error:", e);
    }
  }
